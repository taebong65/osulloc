...
$(document).ready(function(){
	...
	$.getJSON("/osulloc/board/fileList/"+bno+".json",
		function(data){
			var str="";
...
//			}
			...
			for(var i=0; i<list.length; i++){
				...
				str+= "<li data-rno='"+list[i].rno+"'><div><b>" + list[i].rno + "</b><div><b>" + list[i].replyer + "</b></div>" 
				...
				str+= "<button type='button' class='btn btn-primary btn-xl' id='menu' data-rno='"+list[i].rno+"'>:</button>"
				str+= "</li>"
	...
	$("#replyModBtn").on("click", function(){
		...
		//var rno = $(this).data("rno");
		var reply = {rno: $("input[name='rno']").val() ,reply:$("textarea[name='reply']").val()}//함수에는 json타입으로 보내주어야 한다. <=> reDetail과 비교		
		console.log(reply);
		
		//댓글수정함수를 호출
		replyService.reupdate(reply,function(update){ //reply는 함수 선언부의 매개변수로 들어가게 된다. (data : JSON값으로 설정해 두었다.) 
			
			//콜백영역 (댓글수정함수를 호출해서 처리)
